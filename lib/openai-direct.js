  ...
  try {
    ...
    const runResponse = await fetch(`https://api.openai.com/v1/threads/${threadId}/runs`, {
      ...
      body: JSON.stringify({
        ...
        // Remove JSON response format requirement as it needs special message requirements
    ...
    if (assistantMessages.length > 0 && assistantMessages[0].content && assistantMessages[0].content.length > 0) {
      ...
      if (firstContent.type === 'text' && firstContent.text && firstContent.text.value) {
        ...
        return {
          text: "Beklager, jeg modtog et uventet svarformat fra assistenten.",
          success: false
        };
      }
    } else {
      return {
        text: "Beklager, jeg kunne ikke generere et svar.",
        success: false
      };
    }
  } catch (error) {
    ...
    return {
      text: `Fejl ved forbindelse til OpenAI Assistant: ${errorMessage}`,
      success: false,
      error: errorMessage
    };
  }
